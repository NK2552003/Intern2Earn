        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div variants={itemVariants} initial="hidden" animate="visible">
            ...
            <p className="text-base sm:text-lg text-white/60 mb-8">
              Monitor the real-time status of all Upskillify services.
            </p>

            {/* Overall Status */}
            <motion.div
              variants={itemVariants}
              className="inline-flex items-center gap-3 px-6 py-3 rounded-full bg-emerald-500/10 border border-emerald-500/20 mx-auto"
            >
      ...
      <section className="py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-white/5 border-y border-white/8">
        <div className="max-w-5xl mx-auto">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">Service Status</h2>
          </motion.div>
          ...
          >
            {services.map((service, idx) => (
              <motion.div
                key={idx}
                variants={itemVariants}
                className="p-4 sm:p-6 rounded-lg border border-white/8 bg-[#05040f] hover:border-violet-400/30 transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs sm:text-sm text-white/60">Last incident: {service.lastIncident}</p>
                    </div>
                  </div>
                  <div className="flex gap-4 sm:text-right">
                    <div>
                      <p className="text-xs text-white/60">Uptime (30 days)</p>
      ...
      <section className="py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4">Incident History</h2>
            <p className="text-white/60">Recent incidents and maintenance</p>
          </motion.div>
          ...
          >
            {incidents.map((incident, idx) => (
              <motion.div
                key={idx}
                variants={itemVariants}
                className="p-6 sm:p-8 rounded-lg border border-white/8 bg-white/5 hover:border-violet-400/30 transition-all"
              >
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
                  <div>
                    <h3 className="text-lg sm:text-xl font-bold text-white mb-2">{incident.title}</h3>
                    <p className="text-sm text-white/60">{incident.date}</p>
                  </div>
                ...
                <p className="text-white/60">{incident.description}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
          ...
          <motion.div variants={itemVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-lg border border-white/8 bg-[#05040f] focus:outline-none focus:border-violet-500"
              />
              <button className="px-6 py-3 bg-linear-to-r from-primary to-primary/90 text-white rounded-lg font-semibold hover:shadow-lg transition-all">
                Subscribe
              </button>
            </div>
          </motion.div>
